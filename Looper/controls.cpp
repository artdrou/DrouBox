...
void blinkBpm(BpmRunner& bpmRunner)
{
    ...
    if (bpmRunner.IsBeat())
    {
        // On the beat: turn LEDs on briefly
        led1.Set(false);
        if (bpmRunner.IsMeasure()) {
            led2.Set(false);
        }
        updateLeds();
        pulseOn = true;
    ...
    // Turn LEDs off shortly after the beat (e.g., 50ms later)
    if (pulseOn && (now - pulseStartTime) > 50)
    {
        led1.Set(true);
        led2.Set(true);
        updateLeds();
